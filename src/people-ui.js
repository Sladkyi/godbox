...
import { RACES, MATERIALS, STOCKS, RESOURCES, inventoryText, structureName, epochOf } from './civilizations.js';
...
const MEMORY={wood:'Gathered wood',build:'Finished a building',friend:'Spent time with someone close',family:'Started a family',birth:'Became a parent',loss:'Lost a partner',move:'Moved to a new town',defend:'Defended the people',hunt:'Fed on a hunt',steal:'Stole foreign stores',clash:'Clashed with a stranger',disaster:'Survived a disaster',scout:'Scouted a foreign town',siege:'Stormed a building',refuge:'Fled as a refugee',gift:'Saw a gift of the land',herd:'Brought an animal into the fold',trade:'Took a cart to another town',festival:'Shared a feast',marvel:'Watched the sky',weather:'Waited out the weather'};
  ...
  update() {
    ...
    if(this.mode==='list') {
      ...
      if(!u){html=head('STORY ENDED')+'<p>This person no longer lives in the world.</p><button class="text-button full-width" data-person-action="list">Other people</button>';this.renderer.followId=null;}
        ...
        html+=`</div><div class="person-facts"><p><span>Mood</span><b>${mood>70?'Content':mood>40?'Calm':'Needs help'}</b></p><p><span>Town</span><b>${escape(v?`${v.name} · ${epochOf(v.population).name}`:'Wanderer')}</b></p><p><span>Home</span><b>${home?structureName(home,v):'None yet'}</b></p><p><span>Carrying</span><b>${inventoryText(u.inventory)}</b></p><p><span>Stores delivered</span><b>${Math.floor(u.delivered)}</b></p></div><div class="settlement-stock"><span class="eyebrow">TOWN STORES</span>${v?`<div>${STOCKS.map(k=>`<span style="--resource:${RESOURCES[k].color}">${RESOURCES[k].name}<b>${Math.floor(v[k])}</b></span>`).join('')}</div><p>Stage: <b>${epochOf(v.population).name}</b> · New houses: <b>${MATERIALS[v.planMaterial]}</b>${v.lastTrade?` · Last trade: ${(RESOURCES[v.lastTrade.good]?.name??v.lastTrade.good).toLowerCase()}`:''}</p>`:'No town founded yet'}</div><div class="person-family"><span class="eyebrow">FAMILY</span><p>${partner?`Partner: ${link(partner)}`:'No partner yet'}${u.pregnancy>0?' · Expecting a child':''}</p>${kids.length?`<p>Children: ${kids.map(link).join(', ')}</p>`:''}${parents.length?`<p>Parents: ${parents.map(link).join(', ')}</p>`:''}</div><div class="person-skills"><span class="eyebrow">SKILLS · GROW WITH PRACTICE</span><div>${Object.entries(SKILLS).map(([key,label])=>`<span>${label}<b>${u.skills[key].toFixed(1)}</b></span>`).join('')}</div></div>`;
