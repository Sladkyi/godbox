...
export function samplePose(u,time) {
  const moving=u.action!=='cheer'&&((u.path?.length??0)>0||Math.hypot(u.tx-u.x,u.ty-u.y)>.25);
  ...
  if(action==='walk'){if(u.action==='patrol'||u.action==='scout'||u.action==='defend'||u.action==='hunt'||u.action==='siege'){p.rightHand=[.35,1.05,.35];p.tool='spear';}if(u.action==='steal')p.tool='crate';return p;}
  ...
  if(action==='deliver'||action==='steal'){p.lean=-.23;p.leftHand=[-.26,.68,.57];p.rightHand=[.26,.68,.57];p.tool='crate';}
  ...
  if(action==='migrate'){p.headTurn=Math.sin(time)*.3;p.leftHand=[-.4,1.02,0];}
  if(action==='cheer'){
