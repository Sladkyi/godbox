...
import { RACES, RESOURCES, DEPOSIT_KINDS, inventoryText, structureName, MATERIALS } from './civilizations.js';
...
const tools = {
  ...
  sheep: { name: 'Sheep', text: 'Peaceful meadow dwellers. Place them on land.', color: '#e7e1c9', category: 'life' },
  wolf: { name: 'Wolves', text: 'Predators hunt sheep. Nature looks for a balance.', color: '#b6c1d6', category: 'life' },
  rain: { name: 'Rain', text: 'Put out fire, save a dry harvest, and a hungry town may name you patron.', color: '#91cddd', category: 'powers' },
...
const singleTools=[...Object.keys(RACES),'sheep','wolf','inspect',...DEPOSIT_KINDS];
...
$('#app').innerHTML = `
  ...
  <main class="game-area">
    ...
    <aside class="toolbox panel">
      ...
      <div class="selected-tool"><div class="selected-heading"><span id="selected-name">Meadow</span><span class="tool-shortcut">BRUSH</span></div><p id="selected-description"></p></div>
...
function renderTools() {
  $('#tool-grid').innerHTML = Object.entries(tools).filter(([, t]) => t.category === category).map(([id, t]) => `<button class="tool ${id === selectedTool ? 'active' : ''}" data-tool="${id}" aria-pressed="${id === selectedTool}" style="--tool-color:${t.color}" title="${t.text}">${icon(id)}<span>${t.name}</span></button>`).join('');
...
function inspect(x, y) {
  ...
  if (unit) { title = { human: 'Person', sheep: 'Sheep', wolf: 'Wolf' }[unit.kind]; content = `Age: ${Math.floor(unit.age)} · Health: ${Math.round(unit.hp)}%`; }
  ...
  else if(building){title=structureName(building);content=`${RACES[building.race].name} · ${building.complete?'Complete':`Building ${Math.round(building.progress*100)}%`}<br>Spent: ${inventoryText(building.cost)}${Number.isFinite(building.hp)?`<br>Strength: ${Math.round(building.hp)}`:''}`;}
  else if (village) {
    ...
    title = village.name;
    content = `${RACES[village.race].name} · People: ${village.population}<br>${inventoryText(village)}<br>Planning to build with: ${MATERIALS[village.planMaterial]}<br>Land: ${Math.round(world.claimRadius(village))} tiles around the center<br>Scouted: ${neighbors.length?neighbors.map(escapeHtml).join(', '):'nobody yet'}<br>At war with: ${foes.length?foes.map(escapeHtml).join(', '):'nobody'}${friends.length?`<br>Allied with: ${friends.map(escapeHtml).join(', ')}`:''}<br>Faith: ${Math.round(village.cult??0)} · ${faith}${village.omen?`<br>Omen: ${escapeHtml(String(village.omen))}`:''}${memory.length?`<br>Memory: ${memory.map(escapeHtml).join('; ')}`:''}`;
  }
...
function applyAt(p, first = false) {
  ...
  const stamp = [...singleTools,'meteor','lightning','rain'].includes(selectedTool);
  ...
  if (!result && first && singleTools.includes(selectedTool)) toast(DEPOSIT_KINDS.includes(selectedTool)?selectedTool==='fish'?'Place a fishing spot in water next to the shore.':'Resources need open land.':world.units.length >= 500 ? 'This world already has 500 creatures.' : 'Life needs land. Paint ground first.');
...
function clearPointers() {
  ...
  pointers.clear(); gesture = null; pairPrev = null; pairLock = null; pendingOrigin = null; cameraLocked = false; dragging = false; lastPaint = null; lastPoint = null;
}
...
function finishPointer(event) {
  ...
  if (pointers.size === 0) { clearHold(); gesture = null; pairPrev = null; pairLock = null; pendingOrigin = null; cameraLocked = false; dragging = false; lastPaint = null; return; }
...
$('#brush').oninput = event => { brush = Number(event.target.value); renderer.radius = brush; $('#brush-value').textContent = brush; };
